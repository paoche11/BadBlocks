...
import numpy as np
import abc
import T2IShield.ptp_utils as ptp_utils
import T2IShield.seq_aligner as seq_aligner
...
def show_cross_attention(attention_store: AttentionStore, res: int, from_where: List[str], select: int = 0):
    ...
    for i in range(len(tokens)):
        ...
        images.append(image)
        image.save("./val/" + decoder(int(tokens[i])) + ".png")
...
def run_and_display(prompts, controller, latent=None, run_baseline=False, generator=None, save=False, id=0, save_path="./output_images"):
    if run_baseline:
        print("w.o. prompt-to-prompt")
        images, latent = run_and_display(prompts, EmptyControl(), latent=latent, run_baseline=False,
                                         generator=generator)
    ...
    if save:
        ...
        for i, img in enumerate(images):
            ...
            filename = os.path.join(save_path, f"generated_{id}_{i}.png")
